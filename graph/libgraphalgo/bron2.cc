...
namespace cavcom {
  namespace graph {
    ...
    const int Bron2::MODE_ALL;
    const int Bron2::MODE_MAX;
    const int Bron2::MODE_MAX_ONLY;

    Bron2::Bron2(const SimpleGraph &graph, int mode) : GraphAlgorithm(graph), mode_(mode), total_(0), number_(0) {}
